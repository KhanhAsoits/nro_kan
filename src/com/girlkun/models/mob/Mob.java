...
//        nplayer
    ...
    public List<ItemMap> getItemMobReward(Player player, int x, int yEnd) {
        ...
        if (!items.isEmpty()) {
            ...
            // ty le roi skh 1/84 @@@
            // neu roi skh thì 100% la se roi
            if (!player.isPet) {
                List<ItemOptionMobReward> options = item.getOption();
                for (ItemOptionMobReward optionMobReward : options) {
                    if (optionMobReward.getTemp().id >= 127 && optionMobReward.getTemp().id <= 135) {
                        item.setRatio(ConstRatio.ty_le_roi_skh);
                    }
                }
            }
            // chinh ty le roi
