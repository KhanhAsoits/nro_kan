...
//            e.printStackTrace();
    ...
    public void sendItemToPlayer(Player player, int tempId, int quantity, List<ItemOption> options) {
        ...
        if (player != null) {
            if (item != null) {
                ...
                Service.gI().sendThongBao(player, String.format("Bạn vừa nhận được %d %s", quantity, item.template.name));
            }
        }
    }
    ...
    public void chat(Player player, String text) {
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.startsWith("bvp ")) {

                int point_to_buff = Integer.parseInt(text.substring(text.indexOf("q ") + "q ".length(), text.length()).trim());
                String player_name = text.substring(text.indexOf("bvp ") + "bvp ".length(), text.indexOf("q ")).trim();

                if (player_name.equals("")) {
                    Service.gI().sendThongBao(player, "Tên nhân vật không được trống");
                    return;
                }

                if (point_to_buff <= 0) {
                    Service.gI().sendThongBao(player, "Điểm không được nhỏ hơn 0");
                    return;
                }
                ...
                for (Player pl : Client.gI().getPlayers()) {
                    if (player_name.equals(pl.name)) {
                        player_ = pl;
                    }
                }
                if (player_ != null) {
                    player_.getSession().vip_point += point_to_buff;
                    Service.gI().sendThongBao(player, String.format("Buff cho player %s %d Điểm vip.", player_name, point_to_buff));
                    Service.gI().sendThongBao(player_, String.format("Bạn đã được admin buff cho %d Điểm vip.", point_to_buff));
                } else {
            ...
            if (text.equals("admin")) {
                ...
                String itemIdStr = text.replace("i ", "");

                int itemId = Integer.parseInt(itemIdStr.substring(0, itemIdStr.indexOf('q')).trim());

                int quantity = Integer.parseInt(text.replace("i " + itemId + " q", ""));
                ...
                InventoryServiceNew.gI().sendItemBags(player);
                Item item = ItemService.gI().createNewItem(((short) itemId));
                Service.gI().sendThongBao(player, "GET " + item.template.name + " [" + item.template.id + "] " + " [" + quantity + "]" + " SUCCESS !");
    ...
    public void point(Player player) {
        ...
        if (!player.isPet && !player.isBoss && !player.isNewPet) {
            ...
            try {
                ...
                msg.writer().writeInt((int) player.nPoint.hpMax);// hp full
                msg.writer().writeInt(player.nPoint.mpMax);// mp full
                msg.writer().writeInt((int) player.nPoint.hp);// hp
