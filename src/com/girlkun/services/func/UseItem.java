            ...
            switch (item.template.type) {
                ...
                default:
                    switch (item.template.id) {
                        // use gold bar
                        case 457:
                            long plus_gold = (pl.inventory.gold + ConstItem.GOLD_BAR_VALUE) > Inventory.LIMIT_GOLD ? 0 : (pl.inventory.gold + ConstItem.GOLD_BAR_VALUE);
                            if (plus_gold == 0) {
                                Service.gI().sendThongBao(pl, "Vàng sử dụng vượt quá giới hạn.");
                                return;
                            }
                            // value o an goldbar
                            pl.inventory.addGold(ConstItem.GOLD_BAR_VALUE);
                            Service.gI().sendMoney(pl);
                            InventoryServiceNew.gI().subQuantityItemsBag(pl, item, 1);
                            Service.gI().sendThongBao(pl, "Dùng thỏi vàng thành công.");
                            break;
                        ////

                        // dung hop qua huy diet
                        case 2045:
                            NpcService.gI().createMenuConMeo(pl, ConstNpc.MENU_CHON_DO_HUY_DIET, -1, "Chọn đồ hủy diệt muốn mở", "Găng", "Quần", "Áo", "Giày", "Nhẫn");
                            break;
                        case 992:
                            pl.type = 1;
                            pl.maxTime = 5;
