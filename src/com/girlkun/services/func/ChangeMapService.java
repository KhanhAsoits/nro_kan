...
public class ChangeMapService {
    ...
    public void backHome(Player player) {
        ...
        if (home_id != -1) {
            ChangeMapService.gI().changeMapBySpaceShip(player, home_id, -1, -1);
        } else {
            Logger.error("khong ve duoc nha!");
            Service.gI().sendThongBao(player, "Nhà cháy rồi bạn ey....");
        }
    }
        ...
        if (!pl.isAdmin() || !pl.isBoss) {
            if (pl.isDie()) {
                if (pl.haveTennisSpaceShip) {
                    Service.gI().hsChar(pl, (int) pl.nPoint.hpMax, pl.nPoint.mpMax);
                } else {
