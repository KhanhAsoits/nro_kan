...
public class CombineServiceNew {
    ...
    private static final int COST_TACH_DO_THAN = 10;
    ...
    public static final int TACH_DO_THAN = 999;
    ...
    public CombineServiceNew() {
        ...
        this.thanHuyDiet = NpcManager.getNpc(ConstNpc.BILL);
    }
        ...
        switch (player.combineNew.typeCombine) {
            case TACH_DO_THAN:
                if (player.combineNew.itemsCombine.size() < 1) {
                    Service.gI().sendThongBaoOK(player, "Đồ thần đâu tính không làm mà đòi có ăn à!");
                    return;
                }
                List<Item> itemsCombine = player.combineNew.itemsCombine;
                List<Integer> itemsTL = new ArrayList<>(Arrays.asList(555, 557, 559, 556, 558, 560, 562, 564, 566, 563, 565, 567, 561));
                // check item co phai do than linh hay khong
                for (Item it : itemsCombine) {
                    if (!it.isDTL()) {
                        Service.gI().sendThongBaoOK(player, "Có một số đồ không phải đồ thần.");
                        return;
                    }
                }
                this.thanHuyDiet.createOtherMenu(player, ConstNpc.CONFIRM_TACH_DO_THAN, "Tách đồ thần cần 10 điêm vip có xác định tách?", "Đồng ý", "Không");
                break;
            ...
            if (linhthu != null && manhHon != null && manhHon.quantity >= 99) {
                ...
                if (Util.isTrue(player.combineNew.ratioCombine, 100)) {
                    ...
                    if (rdUp == 0) {
                        ...
                        linhthu.itemOptions.add(new Item.ItemOption(14, Util.nextInt(15, 45)));
                    } else if (rdUp == 6) {
    ...
    private void tachDoThan(Player player) {
        if (player.getSession().vip_point > COST_TACH_DO_THAN) {
            List<Item> itemsCombine = player.combineNew.itemsCombine;
            // all valid
            int diem_huy_diet_add = 0;
            for (Item it : itemsCombine) {
                switch (it.template.id) {
                    case 555:
                    case 557:
                    case 559:
                        diem_huy_diet_add += ConstItem.TY_LE_TACH_DO_THAN[0];
                        break;
                    // ao
                    case 556:
                    case 558:
                    case 560:
                        diem_huy_diet_add += ConstItem.TY_LE_TACH_DO_THAN[1];
                        break;
                    // quan
                    case 563:
                    case 565:
                    case 567:
                        diem_huy_diet_add += ConstItem.TY_LE_TACH_DO_THAN[3];
                        break;
                    ///giay
                    case 562:
                    case 564:
                    case 566:
                        // gang
                        diem_huy_diet_add += ConstItem.TY_LE_TACH_DO_THAN[2];
                        break;
                    default:
                        // nhan
                        diem_huy_diet_add += ConstItem.TY_LE_TACH_DO_THAN[4];
                }
            }
            player.getSession().diem_huy_diet += diem_huy_diet_add;
            player.getSession().vip_point -= COST_TACH_DO_THAN;

            // xoa do trong hanh trang
            for (Item item : itemsCombine) {
//                    Inve
                InventoryServiceNew.gI().subQuantityItemsBag(player, item, 1);
            }
            InventoryServiceNew.gI().sendItemBags(player);
            Service.gI().sendThongBao(player, String.format("Bạn nhận được %d điểm hủy diệt", diem_huy_diet_add));
            player.combineNew.itemsCombine.clear();
            reOpenItemCombine(player);
        } else {
            Service.gI().sendThongBao(player, String.format("Bạn không đủ điểm cần %d điểm nữa.", COST_TACH_DO_THAN - player.getSession().vip_point));
        }
    }
...
//                        }
    ...
    private String getTextTopTabCombine(int type) {
        switch (type) {
            case TACH_DO_THAN:
                return "Nhanh nên ta còn đi ngủ";
            case EP_SAO_TRANG_BI:
    ...
    private String getTextInfoTabCombine(int type) {
        switch (type) {
            case TACH_DO_THAN:
                return "Chọn đồ thần\n(Áo, quần, găng, giày hoặc nhẫn) để tách,\nMột đồ thần sẽ được một điểm hủy diệt,\nriêng nhẫn sẽ được 2 điểm";
            case EP_SAO_TRANG_BI:
